...
def debug_manage(dataset):
	a=dir(dataset)
	for key in dir(dataset):
		if "type" in key and "__" not in key:
			setattr(dataset,key,getattr(dataset,key)[:5])
	return dataset

def answer(kbc_path, dataset_hard, dataset_complete, t_norm='min', query_type=QuerDAG.TYPE1_3, candidates=3, scores_normalize = 0, kg_path=None, explain=False):
	#dataset_hard=debug_manage(dataset_hard)
	#dataset_complete=debug_manage(dataset_complete)
