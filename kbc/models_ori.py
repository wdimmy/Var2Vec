...
class KBCModel(nn.Module, ABC):
	...
	def optimize_chains(self, chains: List, regularizer: Regularizer,
						...
						bias1: float = 0.0, bias2: float = 0.0, bias3: float = 0.0):
...
class transE(KBCModel):
	...
	def score(self, x):
		...
		return torch.norm(lhs+rel-rhs,p=1,dim=1,keepdim=True)
	...
	def score_emb(self, lhs_emb, rel_emb, rhs_emb):
		return (torch.norm(lhs_emb + rel_emb - rhs_emb, p=1,dim=1, keepdim=True),
				(lhs_emb, rel_emb, rhs_emb))
	...
	def forward(self, x):
		...
		rel = self.embeddings[1](x[:, 1])
		rhs = self.embeddings[0](x[:, 2])
		to_score = torch.stack([self.embeddings[0].weight],dim=0)
		
		return (torch.norm(lhs+rel-rhs,p=1,dim=-1),(lhs,rel,rhs))

	def forward_emb(self, lhs, rel):
		assert(len(lhs.shape)==2)
		to_score = torch.stack([self.embeddings[0].weight],dim=0)
		return torch.norm(torch.stack([lhs+rel],dim=1).to('cpu')-to_score.to('cpu'),p=1,dim=-1).to(lhs.device)
		...
		with torch.no_grad():
			...
			while c_begin < self.sizes[2]:
				...
				while b_begin < len(queries):
					...
					scores = self.forward_emb(self.embeddings[0](these_queries[:, 0])
											,self.embeddings[1](these_queries[:, 1]))
					targets = self.score(these_queries)
...
class RotatE(KBCModel):
	...
	def score(self, x):
		lhs = self.embeddings[0](x[:, 0])
		rel = torch.chunk(self.embeddings[1](x[:, 1])/(self.init_size/self.pi),2,dim=-1)[0]
		rhs = self.embeddings[0](x[:, 2])
		re_lhs,im_lhs=torch.chunk(lhs,2,dim=-1)
		re_rhs,im_rhs=torch.chunk(rhs,2,dim=-1)
		re_rel=torch.cos(rel)
		im_rel=torch.sin(rel)
		re_score_head = re_lhs * re_rel - im_lhs * im_rel
		im_score_head = re_lhs * im_rel + im_lhs * re_rel
		re_score_head = re_score_head - re_rhs
		im_score_head = im_score_head - im_rhs
		score_1= torch.stack([re_score_head, im_score_head], dim = 0).norm(dim=0)
		re_score_tail = re_rel * re_rhs + im_rel * im_rhs
		im_score_tail = re_rel * im_rhs - im_rel * re_rhs
		re_score_tail = re_score_tail - re_lhs
		im_score_tail = im_score_tail - im_lhs
		score_2= torch.stack([re_score_tail, im_score_tail], dim = 0).norm(dim=0)
	...
	def score_emb(self, lhs_emb, rel_emb, rhs_emb):
		re_lhs,im_lhs=torch.chunk(lhs_emb,2,dim=-1)
		re_rhs,im_rhs=torch.chunk(rhs_emb,2,dim=-1)
		rel_emb=torch.chunk(rel_emb,2,dim=-1)[0]
		rel=rel_emb/(self.init_size/self.pi)
		re_rel=torch.cos(rel)
		im_rel=torch.sin(rel)
		re_score_head = re_lhs * re_rel - im_lhs * im_rel
		im_score_head = re_lhs * im_rel + im_lhs * re_rel
		...
		score_2= torch.stack([re_score_tail, im_score_tail], dim = 0).norm(dim=0)
		score=(score_1+score_2)/2.0
		return (torch.sum(score, 1, keepdim=True),(lhs_emb,rel_emb,rhs_emb))

	def forward(self, x):
		lhs = self.embeddings[0](x[:, 0])
		rel = self.embeddings[1](x[:, 1])
		rhs = self.embeddings[0](x[:, 2])
		res=torch.tensor([],dtype=torch.float64,device=lhs.device)
		to_score = self.embeddings[0].weight
		for i in x.shape[0]:
			lhs_t = torch.stack([self.embeddings[0](x[i][0])],dim=0).expand_as(to_score)
			rel_t = torch.stack([self.embeddings[1](x[i][1])],dim=0).expand_as(to_score)
			rhs_t = self.embeddings[0](x[i][2])
		return ((lhs * rel) @ to_score.transpose(0, 1),
				(lhs, rel, rhs))
