...
sig=True
def sigmoid_wrapped(x,bar=0.0):
	global sig
	if sig == True:
		return torch.sigmoid(x)*(1.0-bar)+bar
	else:
		return x
def wrapped(x,bar=0.0):
	return x*(1.0-bar)+bar
def decompose(x,bar=0.0):
#	return torch.sigmoid(x)
	min=torch.min(x,dim=-1).values.unsqueeze(-1).expand_as(x)
	max=torch.max(x,dim=-1).values.unsqueeze(-1).expand_as(x)
	return (x-min)/(max-min)*(1.0-bar)+bar
def decompose_wrapped(x,bar=0.0):
	return decompose(x.reshape(-1),bar).reshape(-1,1)
class KBCModel(nn.Module, ABC):
	...
	def _optimize_variables(scoring_fn: Callable, params: list, optimizer: str,
		...
		with tqdm.tqdm(total=max_steps, unit='iter', disable=False) as bar:
			...
			while i < max_steps and (i < 100 or min_losses[i-100] - min_losses[i] > 1e-9):
				...
				norm, regularizer, _,_ = scoring_fn()
		...
		with torch.no_grad():
			*_, scores,dists = scoring_fn(score_all=True,para=param_backup)

		return scores, min_loss,min_step,dists
	...
	def calc_dist(self,tar,st):
		return torch.mean(torch.norm(tar-self.embeddings[0](torch.tensor(list(st),dtype=torch.long).cuda()),p=2,dim=1))
	def optimize_chains(self, chains: List, regularizer: Regularizer,
						...
						bias1: float = 0.0, bias2: float = 0.0, bias3: float = 0.0,half: bool=False,
						batchsize: int=-1):
		def scoring_fn(score_all=False,para=[]):
			...
			if matrix == 1:
				...
				obj_guess_1=self.converter(torch.cat((lhs_1,rel_1),1))
				obj_guess_2=self.converter(torch.cat((obj_guess_1,rel_2),1))
				if len(chains)==3:
					obj_guess_3=self.converter(torch.cat((obj_guess_2,rel_3),1))
...
#			atoms = torch.cat((decompose_wrapped(score_1), decompose_wrapped(score_2)), dim=1)
			atoms =torch.cat((sigmoid_wrapped(score_1,bias1), sigmoid_wrapped(score_2,bias2)), dim=1)

			if len(chains) == 3:
				score_3, factors_3 = self.score_emb(obj_guess_2, rel_3, obj_guess_3)
				factors.append(factors_3[2])
				atoms = torch.cat((atoms, sigmoid_wrapped(score_3,bias3)), dim=1)
...
#					atoms = torch.stack((decompose_wrapped(score_1).expand_as(score_2), decompose(score_2)), dim=-1)
					atoms = torch.stack((sigmoid_wrapped(score_1,bias1).expand_as(score_2), sigmoid_wrapped(score_2,bias2)), dim=-1)
					#save_flag=True
					#if save_flag is True:
					#	saved=torch.stack((obj_guess_1,obj_guess_2),dim=0)
					#	if max_steps ==0:
					#		torch.save(saved,'/data/cqd2/type1_2_cqd.pt')
					#	else:
					#		torch.save(saved,'/data/cqd2/type1_2_cqd.pt')
					#import pickle
					#easy_answer=pickle.load(open('/data/cqd/FB15k-q2b/test-easy-answers.pkl','rb'))
					#hard_answer=pickle.load(open('/data/cqd/FB15k-q2b/test-hard-answers.pkl','rb'))
					#mid=pickle.load(open('/data/cqd/FB15k-q2b/test-mid.pkl','rb'))
					#onep_query=torch.load('/data/cqd2/type1_2ind.pt')
					#dist_a=torch.tensor([],dtype=torch.float32).cuda()
					#dist_b=torch.tensor([],dtype=torch.float32).cuda()
					#dist=torch.tensor([],dtype=torch.float32).cuda()
					#for id,q in tqdm.tqdm(enumerate(onep_query)):
					#	mi=mid[q][0]
					#	ans=easy_answer[q]|hard_answer[q]
					#	dist_a=torch.cat((dist_a,self.calc_dist(obj_guess_1[id],mi).reshape(1)),dim=0)
					#	dist_b=torch.cat((dist_b,self.calc_dist(obj_guess_2[id],ans).reshape(1)),dim=0)
					#	dist=torch.cat((dist,torch.norm(obj_guess_1[id]-obj_guess_2[id],p=2).reshape(1)),dim=0)
					#dists=[torch.mean(dist_a).to(torch.float).item(),torch.mean(dist_b).to(torch.float).item(),torch.mean(dist).to(torch.float).item()]

					
				else:
					score_3 = self.forward_emb(obj_guess_2, rel_3)
					atoms = torch.stack((sigmoid_wrapped(score_1,bias1).expand_as(score_3), sigmoid_wrapped(score_2,bias2).expand_as(score_3), sigmoid_wrapped(score_3,bias3)), dim=-1)
					#save_flag=True
					#if save_flag is True:
					#	saved=torch.stack((obj_guess_1,obj_guess_2,obj_guess_3),dim=0)
					#	torch.save(saved,'/data/cqd2/type1_3_cqd.pt')
					#import pickle
					#easy_answer=pickle.load(open('/data/cqd/FB15k-q2b/test-easy-answers.pkl','rb'))
					#hard_answer=pickle.load(open('/data/cqd/FB15k-q2b/test-hard-answers.pkl','rb'))
					#mid=pickle.load(open('/data/cqd/FB15k-q2b/test-mid.pkl','rb'))
					#onep_query=torch.load('/data/cqd2/type1_3ind.pt')
					#dist_a=torch.tensor([],dtype=torch.float32).cuda()
					#dist_b=torch.tensor([],dtype=torch.float32).cuda()
					#dist_c=torch.tensor([],dtype=torch.float32).cuda()
					#dist=torch.tensor([],dtype=torch.float32).cuda()
					#for id,q in tqdm.tqdm(enumerate(onep_query)):
				#		mi=mid[q][0]
					#	mi2=mid[q][1]
					#	ans=easy_answer[q]|hard_answer[q]
					#	dist_a=torch.cat((dist_a,self.calc_dist(obj_guess_1[id],mi).reshape(1)),dim=0)
					#	dist_b=torch.cat((dist_b,self.calc_dist(obj_guess_2[id],mi2).reshape(1)),dim=0)
					#	dist_c=torch.cat((dist_c,self.calc_dist(obj_guess_3[id],ans).reshape(1)),dim=0)
					#	thr=torch.cat((torch.norm(obj_guess_1[id]-obj_guess_2[id],p=2).reshape(1),torch.norm(obj_guess_1[id]-obj_guess_3[id],p=2).reshape(1),torch.norm(obj_guess_2[id]-obj_guess_3[id],p=2).reshape(1)),dim=0)
					#	dist=torch.cat((dist,torch.mean(thr).reshape(1)),dim=0)
					#dists=[torch.mean(dist_a).to(torch.float).item(),torch.mean(dist_b).to(torch.float).item(),torch.mean(dist_c).to(torch.float).item(),torch.mean(dist).to(torch.float).item()]

				all_scores = self.batch_t_norm(atoms, norm_type)

			return t_norm, guess_regularizer, all_scores,dists
		
		if len(chains) == 2:
			lhs_1, rel_1, rel_2 = self.__get_chains__(chains, graph_type=QuerDAG.TYPE1_2.value)
			if half== True:
				lhs_1_a=lhs_1[:lhs_1.shape[0]//2,:]
				rel_1_a=rel_1[:rel_1.shape[0]//2,:]
				rel_2_a=rel_2[:rel_2.shape[0]//2,:]
				lhs_1_b=lhs_1[lhs_1.shape[0]//2:,:]
				rel_1_b=rel_1[lhs_1.shape[0]//2:,:]
				rel_2_b=rel_2[lhs_1.shape[0]//2:,:]
		elif len(chains) == 3:
		...
		if matrix == 0:
			if half==True:
				val_shape=(lhs_1.shape[0]//2,lhs_1.shape[1])
			else:
				val_shape=(lhs_1.shape[0],lhs_1.shape[1])
			Obj_guess_1 = torch.normal(0, self.init_size, val_shape, device=lhs_1.device, requires_grad=True)
			Obj_guess_2 = torch.normal(0, self.init_size, val_shape, device=lhs_1.device, requires_grad=True)
			...
			params=[]
			for name,param in self.converter.named_parameters():
				params.append(param)
		params_ori=params
		scores=torch.tensor([],dtype=torch.float32,device=lhs_1.device)
		b=0
		num_sample=lhs_1.shape[0]
		if batchsize==-1:
			batchsize=num_sample
		time_bg=time.time()
		while(b<num_sample):
			params=copy.deepcopy(params_ori)
			bg=b
			ed=min(num_sample,b+batchsize)
			lhs_1_a=lhs_1[bg:ed]
			rel_1_a=rel_1[bg:ed]
			rel_2_a=rel_2[bg:ed]
			if len(chains)==3:
				rel_3_a=rel_3[bg:ed]
			if matrix==0:
				Obj_guess_1 = torch.normal(0, self.init_size, lhs_1_a.shape, device=lhs_1.device, requires_grad=True)
				Obj_guess_2 = torch.normal(0, self.init_size, lhs_1_a.shape, device=lhs_1.device, requires_grad=True)
				params = [Obj_guess_1, Obj_guess_2]
				if len(chains) == 3:
					Obj_guess_3 = torch.normal(0, self.init_size, lhs_1_a.shape, device=lhs_1.device, requires_grad=True)
					params.append(Obj_guess_3)
			scores_tmp,min_loss,min_step,dists = self._optimize_variables(scoring_fn, params, optimizer, lr, max_steps)
			scores=torch.cat((scores,scores_tmp),dim=0)
			b+=batchsize
		min_loss=time.time()-time_bg
		return scores,min_loss,min_step,dists
	...
	def optimize_intersections(self, chains: List, regularizer: Regularizer,
		...
		def scoring_fn(score_all=False,para=[]):
			...
			atoms = sigmoid_wrapped(torch.cat((score_1, score_2), dim=1))

			if len(chains) == 3:
				score_3, _ = self.score_emb(lhs_3, rel_3, obj_guess)
				atoms = torch.cat((atoms, sigmoid_wrapped(score_3)), dim=1)
			...
			if score_all:
				...
				score_1 = decompose(score_1)
				score_2 = decompose(score_2)

				atoms = torch.stack((score_1, score_2), dim=-1)

				if disjunctive:
					pass
					#atoms = sigmoid_wrapped(atoms)

				if len(chains) == 3:
					score_3 = self.forward_emb(lhs_3, rel_3)
					score_3 = decompose(score_3)
			...
			return t_norm, guess_regularizer, all_scores,None
		...
		scores, min_loss, min_step,dists = self._optimize_variables(scoring_fn, params, optimizer, lr, max_steps)
		return scores,min_loss,min_step,dists
	
	def optimize_intersections_not(self, chains: List, regularizer: Regularizer,
							   max_steps: int = 20, lr: float = 0.1,
							   optimizer:str = 'adam', norm_type: str = 'min',
							   disjunctive=False, matrix: int=0):
		def scoring_fn(score_all=False,para=[]):
			...
			if score_all:
				...
				score_2 = decompose(score_2)
				if len(chains) == 2:
					score_2=torch.tensor(1,dtype=torch.float32)-score_2
				...
				if len(chains) == 3:
					score_3 = self.forward_emb(lhs_3, rel_3)
					score_3 = decompose(score_3)
					score_3=torch.tensor(1,dtype=torch.float32)-score_3
					atoms = torch.cat((atoms, score_3.unsqueeze(-1)), dim=-1)
		...
		scores, min_loss, min_step,_ = self._optimize_variables(scoring_fn, params, optimizer, lr, max_steps)
		return scores,min_loss,min_step,_

	def optimize_3_3(self, chains: List, regularizer: Regularizer,
					 max_steps: int = 20, lr: float = 0.1,
					 optimizer:str = 'adam', norm_type: str = 'min', matrix:int=0,
					 type: str='3_3',bias1: float=0.0,bias2: float=0.0):
		def scoring_fn(score_all=False,para=[]):
			if len(para)==0:
				para=params
			if matrix == 1:
...
#				obj_guess_3=torch.nn.functional.linear(torch.cat((lhs_2,rel_3),1),para[0],para[1])
			...
			factors = [factors_1[2], factors_2[2]]
			relu=torch.nn.ReLU()
			atoms = torch.cat((torch.sigmoid(score_1), torch.sigmoid(score_2), relu(score_3.reshape(-1)).reshape(-1,1)), dim=1)
			...
			if score_all:
				...
				#if save_flag is True:
				#	saved=torch.stack((obj_guess_1,obj_guess_2),dim=0)
				#	torch.save(saved,'/data/cqd2/type3_3.pt')
				if type!='3_3_not':
					score_2=sigmoid_wrapped(score_2,bias2)
				else:
					score_2=wrapped(torch.tensor(1,dtype=torch.float32)-torch.sigmoid(score_2),bias2)
				if type!='3_3_notdown':
					score_3=relu(score_3)
				else:
					score_3=relu(score_3)
				atoms = torch.stack((sigmoid_wrapped(score_1,bias1).expand_as(score_2), score_2, score_3), dim=-1)
		...
		scores,min_loss,min_step,_ = self._optimize_variables(scoring_fn, params, optimizer, lr, max_steps)
		return scores,min_loss,min_step,_
	def optimize_3_3_tmp(self, chains: List, regularizer: Regularizer,
					 max_steps: int = 20, lr: float = 0.1,
					 optimizer:str = 'adam', norm_type: str = 'min', matrix:int=0,
					 type: str='3_3'):
		def scoring_fn(score_all=False,para=[]):
...
#				obj_guess_3=torch.nn.functional.linear(torch.cat((lhs_2,rel_3),1),para[0],para[1])
			...
			factors = [factors_1[2], factors_2[2]]
			relu=torch.nn.ReLU()
			atoms = torch.cat((torch.sigmoid(score_1.reshape(-1)).reshape(-1,1), torch.sigmoid(score_2.reshape(-1)).reshape(-1,1), decompose(score_3.reshape(-1)).reshape(-1,1)), dim=1)
			...
			if score_all:
				...
				if type!='3_3_not':
					...
					score_2=torch.tensor(1,dtype=torch.float32)-torch.sigmoid(score_2)
				if type!='3_3_notdown':
					score_3=decompose(score_3)
				else:
#					score_3=torch.ones(score_3.shape).to(score_3.device)
					score_3=torch.tensor(1,dtype=torch.float32)-decompose(score_3)
					score_3=score_3/20+0.95
				atoms = torch.stack((torch.sigmoid(score_1.reshape(-1)).reshape(-1,1).expand_as(score_2), torch.sigmoid(score_2), score_3), dim=-1)
	...
	def optimize_4_3(self, chains: List, regularizer: Regularizer,
		...
		def scoring_fn(score_all=False,para=[]):
			...
			if matrix == 0:
				...
				if not disjunctive:
					atoms = sigmoid_wrapped(torch.cat((score_1, score_2, score_3), dim=1))
					t_norm = self.batch_t_norm(atoms, norm_type)
				else:
					disj_atoms = sigmoid_wrapped(torch.cat((score_1, score_2), dim=1))
					t_conorm = self.batch_t_conorm(disj_atoms, norm_type).unsqueeze(1)

					conj_atoms = torch.cat((t_conorm, sigmoid_wrapped(score_3)), dim=1)
				...
				if score_all:
					...
					if not disjunctive:
						atoms = sigmoid_wrapped(torch.stack((score_1.expand_as(score_3), score_2.expand_as(score_3), score_3), dim=-1))
					else:
						atoms = torch.stack((t_conorm.expand_as(score_3), sigmoid_wrapped(score_3)), dim=-1)
				...
				if not disjunctive:
					atoms = sigmoid_wrapped(torch.cat((score_1, score_2, score_3,score_4), dim=1))
					t_norm = self.batch_t_norm(atoms, norm_type)
				else:
					atoms_1 = torch.cat((sigmoid_wrapped(score_1,bias1), sigmoid_wrapped(score_3,bias2)), dim=1)
					atoms_2 = torch.cat((sigmoid_wrapped(score_2,bias1), sigmoid_wrapped(score_4,bias2)), dim=1)
				...
				if score_all:
					...
					if not disjunctive:
						atoms = sigmoid_wrapped(torch.stack((score_1.expand_as(score_3), score_2.expand_as(score_3), score_3,score_4), dim=-1))
						all_scores = self.batch_t_norm(atoms, norm_type)
					else:
						atoms_1 = torch.stack((sigmoid_wrapped(score_1,bias1).expand_as(score_3), sigmoid_wrapped(score_3,bias2)), dim=-1)
						atoms_2 = torch.stack((sigmoid_wrapped(score_2,bias1).expand_as(score_4), sigmoid_wrapped(score_4,bias2)), dim=-1)
...
#			mat = torch.normal(0, self.init_size, (lhs_1.shape[1]+rel_1.shape[1],lhs_1.shape[1]), device=lhs_1.device, requires_grad=True)
			params = [self.converter.weight,self.converter.bias]
		params_ori=params
		scores=torch.tensor([],dtype=torch.float32,device='cpu')
		b=0
		num_sample=lhs_1.shape[0]
		if batchsize==-1:
			batchsize=num_sample
	...
	def optimize_4_3_consj(self, chains: List, regularizer: Regularizer,
		...
		def scoring_fn(score_all=False,para=[]):
			...
			if not disjunctive:
				score_3=sigmoid_wrapped(score_3)
				score_1=sigmoid_wrapped(score_1)
				score_2=sigmoid_wrapped(score_2)
				atoms = torch.cat((score_1, score_2, score_3), dim=1)
			...
			if score_all:
				...
				if save_flag is True:
					saved=torch.stack((obj_guess_1,obj_guess_2),dim=0)
					torch.save(saved,'/data/cqd2/type4_3.pt')
				if not disjunctive:
					score_3=sigmoid_wrapped(score_3,bias3)
					score_1=sigmoid_wrapped(score_1,bias1)
					if is_neg:
						score_2=decompose_wrapped(torch.tensor(1,dtype=torch.float32)-decompose(score_2.reshape(-1)).reshape(-1,1),bias2)
						#score_2=torch.ones_like(score_2)
					else:
						score_2=sigmoid_wrapped(score_2,bias2)
					atoms = torch.stack((score_1.expand_as(score_3), score_2.expand_as(score_3), score_3), dim=-1)
				else:
					atoms = torch.stack((t_conorm.expand_as(score_3), sigmoid_wrapped(score_3)), dim=-1)
				all_scores = self.batch_t_norm(atoms, norm_type)
			return t_norm, guess_regularizer, all_scores,None
		...
		if not last_step:
			...
			if env is not None:
				...
				for i in range(z_indices.shape[1]):
					...
					ent_score = sigmoid_wrapped(z_scores[0, i]).item()
						...
						for hop_num, ind in enumerate(indices):
							...
							if f"rhs_{ind}" not in candidate_cache:
								...
								if 'disj' in env.graph_type or scores_normalize:
									z_scores_1d = sigmoid_wrapped(z_scores_1d)
...
class ProjectionMLP(nn.Module):
    def __init__(self, n_layers, entity_dim, init_size):
        super(ProjectionMLP, self).__init__()
        self.n_layers = n_layers
        for i in range(1, self.n_layers+1):
            setattr(self, "proj_layer_{}".format(i), nn.Linear(2 * entity_dim, 2 * entity_dim))
            getattr(self, "proj_layer_{}".format(i)).weight.data *= init_size

        self.last_layer = nn.Linear(2 * entity_dim, entity_dim)
        self.last_layer.weight.data*=init_size

    def forward(self, x):
        m=torch.nn.LeakyReLU()
        for i in range(1, self.n_layers+1):
            x = m(getattr(self, "proj_layer_{}".format(i))(x))
        x = self.last_layer(x)
        return x
...
class ComplEx(KBCModel):
	def __init__(
		...
		if is_matrix > 0:
			self.converter=ProjectionMLP(is_matrix,2*rank,init_size)
		elif is_matrix > -1:
			self.converter=nn.Linear(4*rank,2*rank)
			self.converter.weight.data*=init_size
		if is_2i is True:
			self.tconverter=nn.Linear(8*rank,2*rank)
			self.tconverter.weight.data*=init_size
	...
	def forward(self, x):
		...
		rhs = self.embeddings[0](x[:, 2])
		if self.is_2i:
			lhs_1 = self.embeddings[0](x[:, 0])
			rel_1 = self.embeddings[1](x[:, 1])
			lhs_2 = self.embeddings[0](x[:, 2])
			rel_2 = self.embeddings[1](x[:, 3])
			rhs = self.embeddings[0](x[:, 4])
			pred=self.tconverter(torch.cat((lhs_1,rel_1,lhs_2,rel_2),dim=1))
			loss=torch.norm(pred-rhs,p=2,dim=1)
			lhs_1 = lhs[:, :self.rank], lhs[:, self.rank:]
			rel_1 = rel[:, :self.rank], rel[:, self.rank:]
			lhs_2 = lhs[:, :self.rank], lhs[:, self.rank:]
			rel_2 = rel[:, :self.rank], rel[:, self.rank:]
			pred = pred[:, :self.rank], pred[:, self.rank:]
			return (loss),(torch.sqrt(lhs_1[0] ** 2 + lhs_1[1] ** 2),
			torch.sqrt(rel_1[0] ** 2 + rel_1[1] ** 2),
			torch.sqrt(lhs_2[0] ** 2 + lhs_2[1] ** 2),
			torch.sqrt(rel_2[0] ** 2 + rel_2[1] ** 2),
			torch.sqrt(pred[0] ** 2 + pred[1] ** 2))
		if self.is_matrix>=0:
			pred=self.converter(torch.cat((lhs,rel),dim=1))
			loss=torch.norm(pred-rhs,p=1,dim=1)
			lhs = lhs[:, :self.rank], lhs[:, self.rank:]
			rel = rel[:, :self.rank], rel[:, self.rank:]
			pred = pred[:, :self.rank], pred[:, self.rank:]
	...
	def forward_emb_dist(self,obj):
		to_score = self.embeddings[0].weight
		num=to_score.shape[0]
		numq=obj.shape[0]
		answer=torch.zeros((numq,num),dtype=torch.float32)
		for i in range(numq):
			answer[i]=torch.norm(obj[i]-to_score,p=2,dim=-1)
		return answer
...
class DistMult(KBCModel):
	def __init__(
		...
		if is_matrix > 0:
			...
			self.converter=nn.Linear(2*rank,rank)
			self.converter.weight.data*=init_size
		self.init_size = init_size
	...
	def score_emb(self, lhs_emb, rel_emb, rhs_emb):
		return (torch.sum(lhs_emb * rel_emb * rhs_emb, 1, keepdim=True),
	...
	def forward(self, x):
		...
		if self.is_matrix>=0:
			pred=self.converter(torch.cat((lhs,rel),dim=1))
			loss=torch.norm(pred-rhs,p=2,dim=1)
			return ((loss),(lhs, rel, rhs))
		to_score = self.embeddings[0].weight
		return ((lhs * rel) @ to_score.transpose(0, 1),
				(lhs, rel, rhs))

	def forward_emb(self, lhs, rel):
		#lhs(batch,d) rel(batch,d)
		to_score = self.embeddings[0].weight
		#to_score(d,num_entity)
		return (lhs * rel) @ to_score.transpose(0, 1)
