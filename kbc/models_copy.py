		...
		def scoring_fn(score_all=False,para=[]):
			...
			if score_all:
				if len(chains) == 2:
					...
					atoms = sigmoid_wrapped(torch.stack((score_1.expand_as(score_2), score_2), dim=-1))
				else:
...
#				obj_guess_3=torch.nn.functional.linear(torch.cat((lhs_2,rel_3),1),para[0],para[1])
			...
			if score_all:
				...
				if type!='3_3_notdown':
					...
					score_3=relu(score_3)
				atoms = torch.stack((torch.sigmoid(score_1).expand_as(score_2), score_2, score_3), dim=-1)
		...
		def scoring_fn(score_all=False,para=[]):
			...
			if matrix == 0:
				...
				if not disjunctive:
					...
					atoms_1 = sigmoid_wrapped(torch.cat((score_1, score_3), dim=1))
					atoms_2 = sigmoid_wrapped(torch.cat((score_2, score_4), dim=1))
				...
				if score_all:
					...
					if not disjunctive:
						...
						atoms_1 = sigmoid_wrapped(torch.stack((score_1.expand_as(score_3), score_3), dim=-1))
						atoms_2 = sigmoid_wrapped(torch.stack((score_2.expand_as(score_4), score_4), dim=-1))
		...
		def scoring_fn(score_all=False,para=[]):
			...
			if score_all:
				...
				if not disjunctive:
					...
					score_1=sigmoid_wrapped(score_1)
					if is_neg:
						score_2=torch.tensor(1,dtype=torch.float32)-decompose(score_2.reshape(-1)).reshape(-1,1)
					else:
						score_2=sigmoid_wrapped(score_2)
